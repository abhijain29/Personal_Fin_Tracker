...
OUTSTANDING_LABEL_FILE = os.path.join(
    PROJECT_DIR, "Reference Documents", "Outstanding_Label_Mapping.csv"
)
...
def load_outstanding_label_map():
    """Load outstanding label mapping from CSV."""
    mapping = {}
    if not os.path.exists(OUTSTANDING_LABEL_FILE):
        return mapping
    with open(OUTSTANDING_LABEL_FILE, "r", encoding="utf-8-sig") as f:
        reader = csv.DictReader(f)
        for row in reader:
            normalized = {k.strip().lstrip("\ufeff"): v for k, v in row.items()}
            bank = (normalized.get("Bank") or "").strip()
            variant = (normalized.get("Card Variant") or "").strip()
            label = (normalized.get("Outstanding Label") or "").strip()
            if not bank or not label:
                continue
            key = (bank.lower(), variant.lower())
            mapping[key] = [label]
    return mapping
        ...
        for file in files:
            ...
            try:
                ...
                for r in records:
                    ...
                    bank_name, variant = split_account_variant(r.get("Account", ""))
                    r["Account"] = bank_name
        ...
        if not df_payments.empty:
            df_payments["Current Outstanding Amt"] = df_payments.apply(
                lambda row: statement_due_map.get(
                    (row.get("Account"), row.get("Card Variant"), row.get("Period"))
                ),
                axis=1,
            )
